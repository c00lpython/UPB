...
class BuildWidget(QWidget):
    ...
    def load_build_config(self):
        """Загружает настройки сборки из папки текущего проекта"""
        ...
        if os.path.exists(config_path):
            try:
                ...
                self.log(f"💾 Загружены настройки для {self.current_project}")
                return True
            except Exception as e:
                self.log(f"⚠️ Ошибка загрузки настроек: {e}")
        else:
            self.log(f"📂 Нет сохранённых настроек для {self.current_project}, используем значения по умолчанию")
        return False
    ...
    def reset_build_config(self):
        """Сбрасывает настройки сборки для текущего проекта"""
        if not self.current_project:
            QMessageBox.warning(self, "Ошибка", "Выберите проект")
            return
        ...
        if os.path.exists(config_path):
            try:
                ...
                self.log(f"🔄 Файл настроек удалён для {self.current_project}")
            except Exception as e:
                self.log(f"⚠️ Ошибка удаления настроек: {e}")

        # Сбрасываем поля в значения по умолчанию
    ...
    def update_project_info(self):
        """Обновляет информацию о проекте и загружает его настройки"""
    ...
    def clear_build(self):
        ...
        reply = QMessageBox.question(
            self,
            "Очистка сборки",
            f"Удалить все файлы сборки для '{self.current_project}'?",
